...
class CommandExecutionAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "<int:pk>/kill/",
                self.admin_site.admin_view(self.kill_command_view),
                name="kill_command",
            ),
            path(
                "<int:pk>/output/",
                self.admin_site.admin_view(self.command_output_view),
                name="command_output",
            ),
            path(
                "<int:pk>/retry/",
                self.admin_site.admin_view(self.retry_command_view),
                name="retry_command",
            ),
        ]
    ...
    def kill_link(self, obj):
        if obj.is_running():
            return format_html(
                '<a class="button" href="{}">Kill</a>', f"{obj.id}/kill/"
            )
        return "-"
    ...
    def retry_link(self, obj):
        if obj.status in ("FAILED", "KILLED"):
            return format_html(
                '<a class="button" href="{}">Retry</a>', f"{obj.id}/retry/"
            )
        return "-"
    ...
    def kill_command_view(self, request, pk):
        ...
        if obj.pid:
            try:
                ...
                obj.save()
                self.message_user(
                    request, f"Process {obj.pid} killed.", messages.SUCCESS
                )
            except ProcessLookupError:
                obj.status = "NOT FOUND"
                obj.save()
                self.message_user(
                    request, f"PID {obj.pid} not found.", messages.WARNING
                )
        else:
